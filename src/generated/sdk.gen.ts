...
/**
 * Create a budget line (hierarchical)
 *
 * Creates a stored budget line under `parentId`. Server mints `id`, the internal address reference, and timestamps, they cannot be client-supplied. Naming a server-owned field returns `422 field_read_only`. Emits a durable `budgetLine.created` audit event.
 */
 ...
 *
 * Patches the provider-writable allow-list. Naming a server-owned field (`address`, `budgetId`, `path`, `depth`, timestamps) returns `422 field_read_only` listing offenders, never a silent strip. Emits a durable `budgetLine.updated` audit event.
 */
 ...
 *
 * Move a PO from `draft|rejected` → `pending` and trigger the approval flow (an auto-approve rule may advance it straight to `approved`). From any other status returns `409 po_invalid_status` with a `Cannot submit…` message. If no approval flow is configured the call fails with `409 po_invalid_status`. Emits the `purchaseOrder.pending` webhook event. There is no `/approve` verb, approval is flow-driven.
 */
