 ...
 *
 * Browse/edit individual stored budget lines, paginated. Filters AND-compose. `accountId` is a classifier and returns a set (an account code can sit on many rows); `path` returns exactly one row (or `409 account_path_ambiguous` if a coded path matches more than one live row). For the complete budget document use `GET /budget`. Expand keys: `phases, inputs, contact, account, sourceItem`; `inputs` returns raw editable estimate inputs keyed by phase id. (The legacy `lines.notes` expand is not ported, the note is a plain-text `line.notes` field always present inline; see `BudgetLine.notes`.)
 */
...
/**
 * Create a budget line with optional phase inputs
 *
 * Creates a stored budget line under `parentId`, optionally with initial editable inputs keyed by phase id. Server mints `id`, the internal address reference, and timestamps, they cannot be client-supplied. Naming a server-owned field returns `422 field_read_only`. Emits a durable `budgetLine.created` audit event.
 */
 ...
 *
 * Patches the provider-writable allow-list, optionally upserting editable inputs keyed by phase id. Naming a server-owned field (`address`, `budgetId`, `path`, `depth`, timestamps) returns `422 field_read_only` listing offenders, never a silent strip. Emits a durable `budgetLine.updated` audit event.
 */
...
/**
 * Upsert one editable line input for one phase
 *
 * Upserts raw editable estimate inputs for one line/phase intersection. Omitted fields are left unchanged; explicit `null` clears nullable fields.
 */
export const budgetUpsertLineInput = <ThrowOnError extends boolean = false>(options: Options<BudgetUpsertLineInputData, ThrowOnError>) => (options.client ?? client).put<BudgetUpsertLineInputResponses, BudgetUpsertLineInputErrors, ThrowOnError>({
    security: [{ scheme: 'bearer', type: 'http' }],
 ...
 *
 * Soft-delete a purchase order (sets `soft-delete timestamp`). Allowed only while `status ∈ {draft, rejected}`; otherwise `409 po_invalid_status` (not a silent no-op). Idempotent.
 */
 ...
 *
 * Move a PO from `pending` → `draft` and cancel the live approval flow. From any other status returns `409 po_invalid_status` with a `Cannot cancel submission…` message.
 */
